...
- [x] 3. 建立模擬資料與渲染邏輯
  ...
  **驗收條件**
  - [x] 3.1 四個卡片各顯示 5 筆資料
  - [x] 3.2 點擊任一筆，`.detail` 展開；再點擊，收合
  - [x] 3.3 稽核缺失的「處理中」顯示橙色色塊，「已結案」顯示綠色色塊
  - [x] 3.4 瀏覽器 Console 無任何錯誤
  - [x] 3.5 `app.js` 不超過 150 行，無 `import`／`require`
...
- [x] 4. 整合檢查
  ...
  **驗收條件**
  - [x] 4.1 requirements.md 所有 `[ ]` 已全數改為 `[x]`
  - [x] 4.2 DevTools Network 面板重整後無外部請求
  - [x] 4.3 頁面標題、卡片標題、資料內容全為繁體中文
  - [x] 4.4 三種裝置寬度下版面無破版
